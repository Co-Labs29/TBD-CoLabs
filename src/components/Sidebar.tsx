...
const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 bg-gray-800 bg-opacity-50 z-40 transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={toggleDrawer}></div>

      <aside className={`fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform md:translate-x-0 z-50 md:z-auto bg-white w-64 md:w-40 h-full border-r-2 border-gray-300 flex flex-col items-center justify-between font-semibold`}>
        <div className="mt-5">
